...
from app.api.cardapio.schemas.schema_pedido_status_historico import (
...
router = APIRouter(prefix="/api/cardapio/admin/pedidos", tags=["Admin - Cardápio - Pedidos"], dependencies=[Depends(get_current_user)])
    ...
    return HistoricoDoPedidoResponse(
        ...
        historicos=[
            PedidoStatusHistoricoOut(
                ...
                criado_por=h.criado_por.username if hasattr(h.criado_por, 'username') and h.criado_por else (h.criado_por if isinstance(h.criado_por, str) else None),
