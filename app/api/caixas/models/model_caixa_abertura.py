...
CaixaAberturaStatus = SAEnum("ABERTO", "FECHADO", name="caixa_abertura_status_enum", create_type=False, schema="cadastros")

class CaixaAberturaModel(Base):
    """Armazena informações de abertura e fechamento de caixas"""
    __tablename__ = "caixa_aberturas"
    __table_args__ = (
        Index("idx_caixa_abertura_caixa", "caixa_id"),
        Index("idx_caixa_abertura_empresa", "empresa_id"),
        Index("idx_caixa_abertura_status", "status"),
        Index("idx_caixa_abertura_data_abertura", "data_abertura"),
        {"schema": "cadastros"},
    )
    ...
    # Relacionamentos
    caixa_id = Column(Integer, ForeignKey("cadastros.caixas.id", ondelete="RESTRICT"), nullable=False)
    caixa = relationship("CaixaModel", back_populates="aberturas")
    ...
    # Status e datas
    status = Column(CaixaAberturaStatus, nullable=False, default="ABERTO")
