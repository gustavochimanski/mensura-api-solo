...
@router.get("/api/public/empresas", summary="Buscar códigos das empresas ativas")
def get_empresas_codigos(db: Session = Depends(get_db)):
    repo = EmpresasRepository(db)
    return repo.buscar_codigos_ativos()

@router.get("/api/public/empresas/detalhes", summary="Buscar objetos completos das empresas ativas")
