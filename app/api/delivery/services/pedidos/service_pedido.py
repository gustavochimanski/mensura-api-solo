...
class PedidoService:
    ...
    # ---------------- Impressão ----------------
    def listar_pedidos_pendentes_impressao(self, empresa_id: int) -> list[PedidoKanbanResponse]:
        """
        Lista todos os pedidos com status 'I' (Pendente de Impressão) de uma empresa.
        """
        pedidos = (
            self.db.query(PedidoDeliveryModel)
            .filter(
    ...
    def marcar_pedido_impresso(self, pedido_id: int) -> PedidoResponse:
        """
        ...
        if pedido.status != PedidoStatusEnum.I.value:
            raise HTTPException(
                status.HTTP_400_BAD_REQUEST, 
                f"Pedido deve estar com status 'I' (Pendente de Impressão). Status atual: {pedido.status}"
            )
        
        # Atualiza o status para Em Preparo
        self.repo.atualizar_status_pedido(pedido, PedidoStatusEnum.R.value, motivo="Pedido impresso com sucesso")
        self.repo.commit()
        
        # Recarrega o pedido para retornar dados atualizados
        pedido = self.repo.get_pedido(pedido_id)
        
        logger.info(f"[marcar_pedido_impresso] pedido_id={pedido_id} -> status=R")
