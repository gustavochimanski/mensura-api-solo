...
async def iniciar_pagamento(
    ...
    logger.info(
        f"[Pedidos] Iniciar pagamento - pedido_id={pedido_id} metodo={metodo} gateway={gateway}"
    )
    svc = PedidoService(db)
...
@router.get(
    "/{pedido_id}/pagamentos/{provider_id}",
...
async def consultar_pagamento(
    ...
    logger.info(
        f"[Pedidos] Consultar pagamento - pedido_id={pedido_id} provider_id={provider_id} gateway={gateway}"
    )
    svc = PedidoService(db)
    pedido = svc.repo.get_pedido(pedido_id)
    if not pedido:
