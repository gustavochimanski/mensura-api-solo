...
class KanbanService:
    ...
    def _processar_pedido_delivery(self, p: PedidoUnificadoModel) -> PedidoKanbanResponse:
        ...
        return PedidoKanbanResponse(
            ...
            troco_para=(lambda: (round(float(p.troco_para) - valor_total_calculado, 2) if (getattr(p, "troco_para", None) is not None and (float(p.troco_para) - valor_total_calculado) > 0) else None))(),
    ...
    def _processar_pedido_mesa(self, p: PedidoUnificadoModel) -> PedidoKanbanResponse:
        ...
        return PedidoKanbanResponse(
            ...
            tempo_entrega_minutos=tempo_entrega_minutos,
            troco_para=(lambda: (round(float(p.troco_para) - valor_total_mesa, 2) if (getattr(p, "troco_para", None) is not None and (float(p.troco_para) - valor_total_mesa) > 0) else None))(),
            tipo_pedido="MESA",
            numero_pedido=getattr(p, "numero_pedido", None) or str(p.id),
            mesa_id=p.mesa_id,
            mesa=mesa_obj,
    ...
    def _processar_pedido_balcao(self, p: PedidoUnificadoModel) -> PedidoKanbanResponse:
        ...
        return PedidoKanbanResponse(
            ...
            acertado_entregador=None,
            tempo_entrega_minutos=tempo_entrega_minutos,
            troco_para=(lambda: (round(float(p.troco_para) - valor_total_balcao, 2) if (getattr(p, "troco_para", None) is not None and (float(p.troco_para) - valor_total_balcao) > 0) else None))(),
            tipo_pedido="BALCAO",
            numero_pedido=getattr(p, "numero_pedido", None) or str(p.id),
            mesa_id=p.mesa_id,
            mesa=mesa_obj,
