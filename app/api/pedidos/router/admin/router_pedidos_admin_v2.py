...
router = APIRouter(
    prefix="/api/pedidos/admin/v2",
    tags=["Admin - Pedidos Unificados v2"],
...
def listar_kanban_v2(
    ...
    return svc.listar_kanban(empresa_id=empresa_id, date_filter=date_filter, limit=limit)
...
def atualizar_status_v2(
    pedido_id: int = Path(..., gt=0),
    payload: PedidoStatusPatchRequest = Body(...),
    current_user: UserModel = Depends(get_current_user),
    svc: PedidoAdminService = Depends(get_pedido_admin_service),
):
    return svc.atualizar_status(pedido_id, payload, user_id=current_user.id if current_user else None)
...
def cancelar_pedido_v2(
    pedido_id: int = Path(..., gt=0),
    current_user: UserModel = Depends(get_current_user),
    svc: PedidoAdminService = Depends(get_pedido_admin_service),
):
    return svc.cancelar(pedido_id, user_id=current_user.id if current_user else None)
...
def atualizar_item_v2(
    ...
    payload_data["acao"] = PedidoItemMutationAction.UPDATE
    payload_data["item_id"] = item_id
    mutation = PedidoItemMutationRequest(**payload_data)
