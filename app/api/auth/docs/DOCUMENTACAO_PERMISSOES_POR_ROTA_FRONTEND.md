...
Em qualquer endpoint autenticado do Supervisor:
- `Authorization: Bearer <access_token>`

### 3.3) Obrigatório: escopo de empresa (multi-tenant)
**A partir de agora, o backend valida o tenant (empresa) em todas as rotas autenticadas do Supervisor.**

Isso significa:
- Envie **sempre** `X-Empresa-Id: <empresa_id>` em **todas** as chamadas do Supervisor (GET/POST/PUT/DELETE).
- Alternativa (quando suportado): `?empresa_id=<empresa_id>` via query.

Sem `X-Empresa-Id` (ou `empresa_id`), o backend pode retornar:
- **400**: `empresa_id é obrigatório`
- **403**: `Você não tem acesso a esta empresa`

Observações:
- Rotas públicas e de autenticação (ex.: `POST /api/auth/token`, `GET /api/auth/me`) podem ser exceção e não exigem empresa.
