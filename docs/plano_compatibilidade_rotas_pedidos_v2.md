...
Garantir a convivência controlada entre as rotas legadas de pedidos e a API unificada (`/api/pedidos/admin`), permitindo migração gradual do frontend e de integrações externas sem regressões.
...
| --- | --- | --- | --- |
| 0 – Dev/Staging | Squad backend + QA | Habilitar feature flag da API unificada apenas em dev/staging. Validar com suite automatizada e smoke manual. | Testes de regressão verdes, documentação revisada. |
...
| 2 – Default ON | Todas as empresas | Habilitar novo contrato por padrão; manter canal de suporte e observabilidade reforçada. | Uso de contratos legados < 20% das requisições totais por 2 semanas. |
...
- [x] Implementar router unificado com endpoints descritos em `docs/API_PEDIDOS_UNIFICADOS_ADMIN.md`.
- [x] Ajustar rotas legadas para usar `PedidoAdminService`.
- [ ] Configurar flag/roteamento externo para rollout controlado.
- [ ] Configurar métricas e alertas (observability).
- [ ] Atualizar testes existentes e criar novos cenários para o fluxo unificado.
...
| Risco | Mitigação |
| --- | --- |
| Divergência de comportamento entre legado e contrato unificado | Testes de contrato automatizados comparando respostas para casos críticos. |
